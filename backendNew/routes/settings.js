...
router.get("/settings", adminRequired, async (req, res) => {
  ...
  for (const row of categories) {
    const settings = await Setting.findAll({
      ...
      order: [["settings_key", "ASC"]],
    });
    result[row.category] = await Promise.all(
      settings.map(async (s) => ({
        id: s.id,
        key: s.settings_key,
        value: s.is_encrypted
          ? await Setting.getValue(s.settings_key)
          : s.settings_value,
        type: s.type,
        category: s.category,
  ...
  res.json({
    ...
    setting: {
      ...
      value: setting.is_encrypted
        ? await Setting.getValue(setting.settings_key)
...
router.post("/settings/steadfast/test", adminRequired, async (req, res) => {
  ...
  if (!(await service.isConfigured())) {
    return res.status(400).json({
      error:
        "Steadfast credentials not configured. Please save API key and secret first.",
    });
  }
  try {
    ...
    res.status(400).json({
      success: false,
      message: `Failed to connect to Steadfast: ${err.message}`,
    });
  }
});

router.post(
  "/settings/steadfast/order/:orderId",
  // adminRequired,
  async (req, res) => {
    ...
    if (order.courier_parcel_id)
      return res
        .status(400)
        .json({ error: "Order already has a courier tracking ID" });
    ...
    const recipientAddress = `${order.address}, ${order.upazila_zone || ""}, ${order.district || ""}`;
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Failed to create consignment", details: result });
    } catch (err) {
      return res.status(400).json({
        error: `Failed to create courier consignment: ${err.message}`,
      });
    }
  },
);
