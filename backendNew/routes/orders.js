...
const { Order, OrderStatus, ActivityLog, OrderItem, Media, Setting } = require('../models');
...
router.get('/orders', async (req, res) => {
  ...
  const filtered = status ? orders.filter((order) => {
    const s = order.status;
    if (!s) return false;
    switch (status) {
      case 'design_pending': return s.design_ready === false;
      case 'design_ready': return s.design_ready === true;
      case 'printed': return s.is_printed === true;
      case 'picked': return s.picking_done === true;
      case 'submitted': return s.delivery_status === 'Submitted';
      case 'delivered': return s.delivery_status === 'Delivered';
      case 'returned': return s.delivery_status === 'Returned';
      default: return true;
    }
  }) : orders;

  res.json(await Promise.all(filtered.map(async (order) => {
...
router.post('/orders', async (req, res) => {
  ...
  await logActivity(order.id, 'Order Created', `Customer: ${order.customer_name}`);

  const steadfastResult = { attempted: false, success: false, error: null, consignment_id: null };
  ...
  if (autoCreate && !order.courier_parcel_id) {
    ...
    if (await service.isConfigured()) {
      ...
      const recipientAddress = order.address ? `${order.address}${order.upazila_zone ? `, ${order.upazila_zone}` : ''}${order.district ? `, ${order.district}` : ''}` : `${order.upazila_zone}, ${order.district}`;
      ...
      try {
        ...
        if (result.status === 200 && result.consignment) {
          ...
          await logActivity(order.id, 'Steadfast Consignment Created', `Consignment ID: ${result.consignment.consignment_id}, Tracking: ${result.consignment.tracking_code || 'N/A'}`);
        } else {
          steadfastResult.error = result.message || 'Unknown error';
          await logActivity(order.id, 'Steadfast Creation Failed', `Failed: ${steadfastResult.error}`);
        }
      } catch (err) {
        steadfastResult.error = err.message;
        await logActivity(order.id, 'Steadfast Creation Error', `Exception: ${err.message}`);
      }
    } else {
      steadfastResult.error = 'Steadfast API credentials not configured';
      await logActivity(order.id, 'Steadfast Skipped', 'Steadfast not configured (missing API credentials)');
    }
  } else if (!autoCreate) {
...
router.get('/orders/:orderId', async (req, res) => {
  ...
  if (includeItems) {
    include.push({ model: OrderItem, as: 'items', include: [{ model: Media, as: 'media' }] });
  }

  const order = await Order.findByPk(req.params.orderId, { include });
  if (!order) return res.status(404).json({ error: 'Order not found' });
  res.json(order.toJson({ with_status: true, with_media: true, with_items: includeItems }));
});
...
router.put('/orders/:orderId/status', async (req, res) => {
  const order = await Order.findByPk(req.params.orderId, { include: [{ model: OrderStatus, as: 'status' }] });
  if (!order) return res.status(404).json({ error: 'Order not found' });
  ...
  if (Array.isArray(req.body.design_files) && req.body.design_files.length > 0) {
    let existing = [];
    try {
    ...
    updates.push(`Design files added: ${req.body.design_files.length} (total: ${combined.length})`);
  }
  ...
  const finalOrder = await Order.findByPk(order.id, { include: [{ model: OrderStatus, as: 'status' }] });
...
router.post('/orders/:orderId/design-upload', tokenRequired, upload.array('files'), async (req, res) => {
  const order = await Order.findByPk(req.params.orderId);
  if (!order) return res.status(404).json({ error: 'Order not found' });
  ...
  let item = null;
  if (itemId) {
    item = await OrderItem.findOne({ where: { id: itemId, order_id: order.id } });
  ...
  if (config.cloudinary.enabled) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const media = await createMedia(file, fileType, result.secure_url, result.public_id);
...
router.post('/orders/bulk-delete', async (req, res) => {
  const orderIds = Array.isArray(req.body.order_ids) ? req.body.order_ids.map((id) => Number(id)).filter((id) => !Number.isNaN(id)) : [];
...
router.post('/orders/:orderId/items', async (req, res) => {
  const order = await Order.findByPk(req.params.orderId);
  if (!order) return res.status(404).json({ error: 'Order not found' });
  if (!req.body.size) return res.status(400).json({ error: 'size is required' });
  if (req.body.quantity == null) return res.status(400).json({ error: 'quantity is required' });
  const quantity = parseInt(req.body.quantity, 10);
  if (Number.isNaN(quantity) || quantity < 1) return res.status(400).json({ error: 'quantity must be a positive integer' });
...
router.put('/orders/:orderId/items/:itemId', async (req, res) => {
  const item = await OrderItem.findOne({ where: { id: req.params.itemId, order_id: req.params.orderId } });
  if (!item) return res.status(404).json({ error: 'Item not found' });
...
router.delete('/orders/:orderId/items/:itemId', async (req, res) => {
  ...
  if (!item) return res.status(404).json({ error: 'Item not found' });
  await item.destroy();
  await logActivity(req.params.orderId, 'Item Deleted', `Item ${item.id} removed`);
...
const generatePrintHtml = (orders, baseUrl, columns) => {
  ...
  for (const order of orders) {
    ...
    if (cols.phone_number) html += `<div class="order-detail-item"><span class="order-detail-label">Phone:</span> ${escapeHtml(order.phone_number)}</div>`;
    if (cols.address) {
      const addressParts = [order.address, order.upazila_zone, order.district, order.division].filter(Boolean);
      html += `<div class="order-detail-item"><span class="order-detail-label">Address:</span> ${escapeHtml(addressParts.join(', '))}</div>`;
    }
    if (cols.description) html += `<div class="order-detail-item"><span class="order-detail-label">Description:</span> ${escapeHtml(order.description)}</div>`;
    if (cols.price) html += `<div class="order-detail-item"><span class="order-detail-label">Price:</span> ৳${escapeHtml(order.price)}</div>`;
    if (cols.payment_type) html += `<div class="order-detail-item"><span class="order-detail-label">Payment:</span> ${escapeHtml(order.payment_type)}</div>`;
    if (cols.courier_parcel_id) html += `<div class="order-detail-item"><span class="order-detail-label">Courier Parcel ID:</span> ${escapeHtml(order.courier_parcel_id)}</div>`;
    if (cols.created_at) html += `<div class="order-detail-item"><span class="order-detail-label">Created:</span> ${escapeHtml(order.created_at ? new Date(order.created_at).toISOString().slice(0, 10) : 'N/A')}</div>`;
    html += `</div></div>`;
    if (cols.attachments) {
    ...
    if (cols.items) {
      ...
      for (const item of order.items || []) {
        const frontImages = (item.media || []).filter((m) => m.side === 'front');
