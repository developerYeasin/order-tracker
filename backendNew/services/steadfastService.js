...
class SteadfastService {
  ...
  async _makeRequest(method, endpoint, data = null, params = null) {
    ...
    const response = await axios({ method, url, data, params, headers, timeout: 30000 });
  ...
  normalizePhone(phoneRaw) {
    ...
    if (!/^01\d{9}$/.test(phoneDigits)) {
      throw new Error('recipient_phone must be a valid Bangladeshi mobile number (11 digits starting with 01)');
    }
    return phoneDigits;
  }

  async createOrder(orderData) {
    const required = ['invoice', 'recipient_name', 'recipient_phone', 'recipient_address', 'cod_amount'];
    ...
    if (orderData.recipient_email) payload.recipient_email = String(orderData.recipient_email);
    if (orderData.alternative_phone) payload.alternative_phone = String(orderData.alternative_phone);
    if (orderData.note) payload.note = String(orderData.note).slice(0, 500);
    if (orderData.item_description) payload.item_description = String(orderData.item_description).slice(0, 500);
    if (orderData.total_lot != null) payload.total_lot = Number(orderData.total_lot);
    if (orderData.delivery_type != null) payload.delivery_type = Number(orderData.delivery_type);

    return this._makeRequest('POST', '/create_order', payload);
  }
  ...
  async bulkCreateOrders(ordersData) {
    ...
    return this._makeRequest('POST', '/create_order/bulk-order', { data: ordersData });
  }
  ...
  async createReturnRequest({ consignment_id, invoice, tracking_code, reason }) {
